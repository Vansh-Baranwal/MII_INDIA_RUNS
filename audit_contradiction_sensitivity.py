...
target_candidates = ["CAND_0018499", "CAND_0019480", "CAND_0092278", "CAND_0077337", "CAND_0083307"]

def simulate_contradiction_weight(name, coeff):
    df_sim = df_pool.with_columns(
        ((-coeff * pl.col("contradiction_score")).exp()).alias("Honeypot_Decay")
    )
    ...
    print(f"\n{'='*100}")
    print(f"  {name}  (Coefficient: {coeff:.2f})")
    print(f"{'='*100}")
    ...
    for t, c in Counter(titles100).most_common(10):
        print(f"    {t}: {c}")
    ...
    for row in top20.iter_rows(named=True):
        print(f"    {row['rank']+1:>2} | {row['candidate_id']} | {row['current_title']:<35} | Contra={row['contradiction_score']:4.1f} | Final={row['Final_Score']:.4f}")

scenarios = [
    ("Scenario E Baseline", 1.0),
    ("Variant A", 0.20),
    ("Variant B", 0.15),
    ("Variant C", 0.10),
]
...
for name, wt in scenarios:
    simulate_contradiction_weight(name, wt)
