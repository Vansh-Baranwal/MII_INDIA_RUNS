...
# Base Variant D integration
base_integration = (1.0 + 0.20 * pl.col("Base_Score"))
base_mult_prefix = pl.col("feat_verified_search_skill") * pl.col("Trajectory_Multiplier") * pl.col("Behavioral_Multiplier") * pl.col("Persona_Penalty") * pl.col("Honeypot_Decay") * base_integration

variants = {
    "Scenario A (Current 3-Feature)": (1.0 + 0.25*pl.col("feat_search_relevance_evidence") + 0.20*pl.col("feat_ranking_depth") + 0.15*pl.col("feat_retrieval_depth") + 0.15*pl.col("feat_evaluation_rigor") + 0.35*pl.col("feat_builder_score")),
    "Scenario B (SearchBuilder Only)": (1.0 + 0.25*pl.col("feat_search_relevance_evidence") + 0.70*pl.col("feat_search_builder")),
    "Scenario C (SearchBuilder + Eval)": (1.0 + 0.25*pl.col("feat_search_relevance_evidence") + 0.70*pl.col("feat_search_builder") + 0.15*pl.col("feat_evaluation_rigor")),
}

print("\n" + "="*120)
print("FEATURE REPLACEMENT EXPERIMENTS")
print("="*120)

targets = ["CAND_0018499", "CAND_0092278", "CAND_0030953", "CAND_0083307"]

for v_name, expr in variants.items():
    df_pool = df_pool.with_columns(
        (expr * base_mult_prefix).alias(f"Final_{v_name}")
    )
    ...
    print(f"\n{v_name}")
    print(f"  Top 100 Metrics:")
    print(f"    Avg Builder:   {b_mean:.3f}")
    print(f"    Avg Retrieval: {r_mean:.3f}")
    print(f"    Avg Ranking:   {rd_mean:.3f}")
    
    print(f"  Top 100 Personas:")
    print(f"    Search Engs: {search} | RecSys Engs: {recsys} | NLP Engs: {nlp} | Applied ML: {aml} | Junior: {junior} | CV: {cv}")
