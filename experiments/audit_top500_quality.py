...
variants = {
    "A) Current optimized pipeline": pl.col("Base_Score") * pl.col("Core_Mult"),
    "B) Variant D (1+0.2*Base) Bonus": pl.col("Core_Mult") * (1.0 + 0.20 * pl.col("Base_Score")),
}

print("\n" + "="*120)
print("TOP 500 QUALITY AUDIT")
print("="*120)
...
for v_name, expr in variants.items():
    ...
    top500 = df_ranked.head(500)
    
    b_mean = top500['feat_builder_score'].mean()
    r_mean = top500['feat_retrieval_depth'].mean()
    rd_mean = top500['feat_ranking_depth'].mean()
    
    titles500 = top500['current_title'].fill_null("").to_list()
    search = sum(1 for t in titles500 if any(k in t.lower() for k in ['search', 'ranking', 'relevance', 'ir engineer', 'information retrieval']))
    recsys = sum(1 for t in titles500 if any(k in t.lower() for k in ['recommendation', 'recsys']))
    nlp = sum(1 for t in titles500 if any(k in t.lower() for k in ['nlp', 'natural language']))
    junior = sum(1 for t in titles500 if 'junior' in t.lower() or 'associate' in t.lower())
    cv = sum(1 for t in titles500 if any(k in t.lower() for k in ['computer vision', 'vision']))
    ...
    e20 = len(top20.filter(pl.col("feat_search_builder") >= 0.80))
    e50 = len(top50.filter(pl.col("feat_search_builder") >= 0.80))
    e100 = len(top100.filter(pl.col("feat_search_builder") >= 0.80))
    e500 = len(top500.filter(pl.col("feat_search_builder") >= 0.80))
    
    print(f"\n{v_name}")
    print(f"  Top 500 Metrics:")
    print(f"    Avg Builder:   {b_mean:.3f}")
    print(f"    Avg Retrieval: {r_mean:.3f}")
    print(f"    Avg Ranking:   {rd_mean:.3f}")
    
    print(f"  Top 500 Personas:")
    print(f"    Search Engs: {search} | RecSys Engs: {recsys} | NLP Engs: {nlp} | Junior: {junior} | CV: {cv}")
    
    print(f"  Elite Concentration (SearchBuilder >= 0.80):")
    print(f"    Top 20:  {e20} elites")
    print(f"    Top 50:  {e50} elites")
    print(f"    Top 100: {e100} elites")
    print(f"    Top 500: {e500} elites")
    print("-" * 120)
