...
# Note: Since features.parquet is being rebuilt with the clean dictionary, 
# feat_ranking_depth is now the clean ranking depth.

# 1. Create feat_search_builder
df_pool = df_pool.with_columns(
...
target_candidates = ["CAND_0092278", "CAND_0077337", "CAND_0083307", "CAND_0018499"]

def simulate_search_builder(name, traj_col):
    df_sim = df_pool.with_columns(
        (0.50 + 0.50 * pl.col(traj_col)).alias("Trajectory_Multiplier")
    )
    ...
    sb_mean = top100['feat_search_builder'].mean()
    ...
    for row in top20.iter_rows(named=True):
        print(f"    {row['rank']+1:>2} | {row['candidate_id']} | {row['current_title']:<35} | B={row['feat_builder_score']:.2f} | RD={row['feat_ranking_depth']:.2f} | SB={row['feat_search_builder']:.2f} | Final={row['Final_Score']:.4f}")

simulate_search_builder("Current Scenario E", "feat_builder_score")
simulate_search_builder("Search Builder Variant", "feat_search_builder")
